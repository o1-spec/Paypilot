...
export default function ErrorBoundary({
  ...
  useEffect(() => {
    // Log the error to console for sandbox debugging
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between overflow-x-hidden selection:bg-indigo-500 selection:text-white font-sans relative">
      ...
      <main className="container mx-auto px-6 py-20 flex flex-col items-center justify-center text-center z-10 grow max-w-md space-y-6 animate-in fade-in duration-300">
        <div className="h-16 w-16 bg-slate-900 border border-slate-800 rounded-3xl flex items-center justify-center text-rose-500 shadow-inner animate-pulse">
          <AlertOctagon className="h-8 w-8" />
        </div>

        <div className="space-y-2">
          <span className="text-[10px] font-bold text-rose-500 uppercase tracking-widest">Application Crash</span>
          <h1 className="text-3xl font-extrabold text-white tracking-tight font-sans">Something went wrong</h1>
          <p className="text-xs text-slate-450 leading-relaxed font-semibold">
            An unexpected error occurred in the sandbox client execution stream.
          </p>
        </div>

        {error.message && (
          <div className="w-full text-left rounded-xl bg-rose-950/30 border border-rose-900/50 p-4 text-[10px] text-rose-455 font-mono break-all leading-normal select-all">
        ...
        <div className="pt-2">
          <button
            onClick={() => reset()}
            className="inline-flex items-center gap-2 rounded-xl bg-indigo-606 hover:bg-indigo-550 text-xs font-bold text-white py-3 px-6 shadow-md transition-all active:scale-98"
          >
      ...
      {/* Footer */}
      <footer className="container mx-auto px-6 py-8 border-t border-slate-900/60 z-10 text-center text-[10px] text-slate-500 font-semibold font-sans">
        PayPilot Platform &copy; 2026. Error logging console active.
      </footer>
    </div>
  );
}
